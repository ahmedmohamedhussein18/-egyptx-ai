...
// Find where getAttractionSvg is defined
const startIdx = text.indexOf('function getAttractionSvg(name_en: string, category: string) {');
// Find where getAttractionSvg ends (before `export default function ExploreContent() {`)
const endIdx = text.indexOf('export default function ExploreContent() {');
...
// Provide consistent generic fallbacks
let genericIndex = 0;
...
function getAttractionImage(name_en: string, category: string) {
  ...
  else if (name.includes('citadel') || name.includes('saladin')) matchedImage = "citadel-in-Cairo-Tours-in-Egypt.jpg"; // or The-Citadel-of-Saladin
  ...
  else if (name.includes('hurghada')) matchedImage = "hurghada.jpg"; // or Hurghada-Beaches
  ...
  else if (name.includes('luxor')) matchedImage = "luxor.jpg"; // or wide-view-luxor
  ...
  else if (name.includes('aswan')) matchedImage = "aswan.jpg";
  else if (category === 'beach' || category === 'nature') matchedImage = "V-SSH_-Swimming-Pool-12-1.webp";

  // If no match found, pick from generic pool
  if (!matchedImage) {
...
let newText = text.substring(0, startIdx) + replacement + text.substring(endIdx);
newText = newText.replace('{getAttractionSvg(dest.name_en, dest.category)}', '{getAttractionImage(dest.name_en, dest.category)}');

fs.writeFileSync('src/components/ExploreContent.tsx', newText);
console.log('Successfully replaced getAttractionSvg with smart getAttractionImage mapping.');
