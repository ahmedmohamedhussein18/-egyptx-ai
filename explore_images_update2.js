...
// safely slice the file
const lines = text.split('\\n');
let s = -1, e = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('function getAttractionSvg')) s = i;
  if (lines[i].includes('function WeatherBadge')) e = i;
}

if (s !== -1 && e !== -1) {
  const newLines = [...lines.slice(0, s), replacement, ...lines.slice(e-1)]; // e-1 to include the empty line before WeatherBadge
  let newText = newLines.join('\\n');
  newText = newText.replace('{getAttractionSvg(dest.name_en, dest.category)}', '{getAttractionImage(dest.name_en, dest.category)}');
  fs.writeFileSync('src/components/ExploreContent.tsx', newText, 'utf8');
  console.log('Fixed ExploreContent successfully.');
} else {
  console.log('Could not find the bounds', s, e);
}
